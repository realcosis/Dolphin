...
{
    [Singleton]
    class UserRelationshipsMessageEvent(IUsersManager usersManager) : IIncomingHandler
    {
        async Task IIncomingHandler.Handle(ClientSession client, IncomingPacket packet)
        {
            ...
            if (user != default)
                await client.Send(new RelationshipStatusInfoMessageComposer(user.Friends, userId));
        }
    }
}